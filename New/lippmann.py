...
def lippmann_transform(lambdas, spectrum, depths, r=-1):
...
#    ax.plot(lam, spectrum, '--k', linewidth=0.5, zorder=-1, alpha=0.5, dashes=(2,2))
    ...
    if not nolabel and short_display:
        ...
        ax.set_xticklabels([400, 'Wavelength ($nm$)', 700])
        ax.set_xlabel(label)
...
def generate_wavelengths(N=300, omega_low=None, c=299792458):
...
def generate_wavelengths_sinc(Z, omega_low=None, symmetric=False, c=299792458):
...
#    lippmann /= np.max(lippmann)
    ...
    spectrum_reconstructed = inverse_lippmann(lippmann, lambdas, depths)
    spectrum_filtered =  np.abs( fda.apply_h(spectrum, Z, lambdas, lambdas, r=r) )**2
#    spectrum_reconstructed = inverse_lippmann(lippmann, lambdas/n0, depths, symmetric=False)
...
#    plt.title('Lippmann retransform')    
    ...
    plt.figure()
    show_spectrum(lambdas, spectrum_filtered, ax=plt.gca()) 
    plt.title('Spectrum filtered') 
    
    plt.figure()
    show_spectrum(lambdas, filtered_reverse, ax=plt.gca()) 
    plt.title('Spectrum filtered reversed') 
...
#    plt.title('Spectrum cleaned') 
    ...
    plt.figure()
    show_spectrum(lambdas, spectrum_reverse, ax=plt.gca()) 
    plt.title('Spectrum reversed') 
    
    plt.figure()
    show_lippmann_transform(depths, lippmann_transform(lambdas, spectrum_reverse, depths)[0], ax=plt.gca()) 
    plt.title('Lippmann reversed')
    
    plt.figure()
    show_lippmann_transform(depths, lippmann_reverse, ax=plt.gca()) 
    plt.title('Lippmann reversed')
    
    plt.figure()
    show_spectrum(lambdas, inverse_lippmann(lippmann_reverse, lambdas, depths), ax=plt.gca()) 
    plt.title('Lippmann reversed inversed')
